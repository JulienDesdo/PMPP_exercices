...
During development, `PresentMon` initially worked correctly when launched manually against the Minecraft `javaw.exe` process. The first real failure was a permissions issue: when the exporter launched `PresentMon` from a non-elevated `PowerShell` session, `PresentMon` could start as a process but failed to create its ETW trace session with access denied. The exporter initially hid that error because `PresentMon`'s stderr was redirected to stdout and every line before the CSV header was silently ignored. After adding temporary logging, the permission error became visible. During further debugging, `PresentMon` was started and stopped many times both manually and by the exporter, using different ETW session names, and at one point two `PresentMon` processes were running simultaneously. One terminal was also closed abruptly because `Ctrl+C` could not be used conveniently from that elevated session. At that point, the exporter handled `Ctrl+C` by calling its `PresentMonReader.stop()` method, which terminated the `PresentMon` subprocess, waited for it, and killed it only if necessary. However, an abrupt termination of the Python process can bypass that cleanup path, so a `PresentMon` process or ETW tracing state may remain behind. The exporter also creates a dedicated session name (`MinecraftGpuLab_<PID>`) and uses `--stop_existing_session` when starting a capture. After the abrupt shutdown and repeated manual tests, `PresentMon` began reporting tens of thousands of lost ETW events and stopped producing CSV frames even when run independently of the exporter. Increasing the `PresentMon` circular buffer did not fix the issue. A full Windows restart reset the tracing environment; afterward, the same direct `PresentMon` command immediately produced frame data again, and the exporter successfully reported CSV stream started and `minecraft_presentmon_up = 1`. We cannot prove the exact internal ETW failure, but the observed evidence is consistent with a stale or conflicting tracing state created during debugging rather than a bug in the CSV parser itself.


Two cleanup bugs were later identified in the exporter itself. First, the `KeyboardInterrupt` handler originally covered only the sampling code, while `time.sleep()` remained outside the `try` block. If `Ctrl+C` occurred during sleep, `presentmon.stop()` could therefore be bypassed. The main loop was changed to use an outer `try/except/finally`, ensuring that cleanup always runs.

A second problem remained inside `PresentMonReader.stop()`: the exporter terminated the `PresentMon.exe` subprocess, but did not explicitly stop the named ETW session (`MinecraftGpuLab_<PID>`). This was confirmed with `logman query -ets`, which showed old MinecraftGpuLab sessions still running even though no corresponding PresentMon process existed. The cleanup logic was therefore changed to terminate the named ETW session first, wait for PresentMon to exit normally, and only use `terminate()` or `kill()` as a fallback. After this change, repeated stop/start cycles returned directly to `CSV stream started` without leaving stale ETW sessions behind.
